...
export const LinkEntryScreen: React.FC<LinkEntryScreenProps> = ({
  ...
  // Auto-detect URL parameter on index load
  React.useEffect(() => {
    try {
      const params = new URLSearchParams(window.location.search);
      const urlParam = params.get('url') || params.get('playlist') || params.get('list');
      if (urlParam) {
        setUrlInput(urlParam);
        handleFetchCourse(urlParam);
      }
    } catch (e) {
      // ignore
    }
  ...
  return (
    <div className="flex-1 bg-[#050505] text-zinc-100 overflow-y-auto p-4 sm:p-8 flex flex-col justify-between">
      <div className="max-w-4xl mx-auto w-full space-y-8 py-4 sm:py-8">
        ...
        {/* Quick jump to existing courses / saved library & Direct Index Catalog */}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h2 className="text-sm font-bold text-white flex items-center gap-2">
                    Sua Biblioteca de Cursos Salvos ({existingCourses.length})
                  </h2>
                  <p className="text-[11px] text-zinc-400">
                    Acesse qualquer aula ou continue de onde parou direto pelo index.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="px-3 py-1.5 bg-[#0e0e12] border border-zinc-800 rounded-lg text-xs text-zinc-200 placeholder-zinc-500 outline-none focus:border-zinc-700 w-full sm:w-56"
                />
              </div>
            </div>

            {/* Grid of Courses directly on index */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {existingCourses
                .filter(c => 
                  !indexSearch.trim() || 
                  c.title.toLowerCase().includes(indexSearch.toLowerCase()) || 
                  c.author.toLowerCase().includes(indexSearch.toLowerCase())
                )
                .map(course => {
                  const completedCount = course.lessons.filter(l => l.completed).length;
                  const totalCount = course.lessons.length;
                  const pct = totalCount > 0 ? Math.round((completedCount / totalCount) * 100) : 0;
                  const isCurrentActive = activeCourse?.id === course.id;

                  return (
                    <div
                      key={course.id}
                      onClick={() => onOpenExistingCourse(course.id)}
                      className={`group relative rounded-xl bg-[#0c0c0f] hover:bg-[#111116] border transition-all cursor-pointer overflow-hidden p-3.5 flex flex-col justify-between ${
                        isCurrentActive ? 'border-emerald-500/50 shadow-lg shadow-emerald-500/5' : 'border-zinc-800/80 hover:border-zinc-700'
                      }`}
                    >
                      <div className="flex items-start gap-3">
                        <div className="w-16 h-12 rounded-lg bg-zinc-900 overflow-hidden shrink-0 relative border border-zinc-800">
                          <img
                        ...
                        <div className="min-w-0 flex-1">
                          <span className="text-[10px] font-medium text-emerald-400 truncate block">
                            {course.author}
                          </span>
                          ...
                          <span className="text-[10px] text-zinc-500 mt-0.5 block">
                            {completedCount} de {totalCount} aulas • {pct}%
                          </span>
                        </div>
                      </div>

                      {/* Progress bar */}
                      <div className="mt-3 pt-2 border-t border-zinc-800/60 flex items-center justify-between gap-3">
                        ...
                        <span className="text-[10px] font-bold text-zinc-400 group-hover:text-emerald-400 flex items-center gap-1">
                          <span>Assistir</span>
