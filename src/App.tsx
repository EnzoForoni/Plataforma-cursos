...
export default function App() {
  ...
  // Handle URL query parameters on initial index load (e.g. ?view=... or ?course=... or ?url=...)
  useEffect(() => {
    try {
      const params = new URLSearchParams(window.location.search);
      const requestedView = params.get('view');
      const requestedCourseId = params.get('course') || params.get('courseId');
      const requestedPlaylist = params.get('url') || params.get('playlist') || params.get('list');

      if (requestedView && ['welcome', 'player', 'library', 'stats'].includes(requestedView)) {
        setCurrentView(requestedView as any);
      }

      if (requestedCourseId) {
        const found = db.courses.find(c => c.id === requestedCourseId);
        if (found) {
          setDb(prev => ({ ...prev, activeCourseId: requestedCourseId }));
          setCurrentView('player');
        }
      }

      if (requestedPlaylist) {
        // Automatically switch to welcome index and trigger auto-fetch if needed
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-zinc-100 flex flex-col selection:bg-emerald-500 selection:text-zinc-950">
      ...
      <nav className="md:hidden sticky bottom-0 z-40 bg-[#070709]/95 backdrop-blur-md border-t border-zinc-800/80 px-2 py-2 flex items-center justify-around">
        <button
          type="button"
          onClick={() => setCurrentView('welcome')}
          className={`flex flex-col items-center gap-1 px-3 py-1 rounded-lg text-[10px] font-semibold transition-colors cursor-pointer ${
            currentView === 'welcome' ? 'text-emerald-400 font-bold' : 'text-zinc-400 hover:text-zinc-200'
          }`}
        >
        ...
        <button
          ...
          className={`flex flex-col items-center gap-1 px-3 py-1 rounded-lg text-[10px] font-semibold transition-colors cursor-pointer ${
            currentView === 'library' ? 'text-emerald-400 font-bold' : 'text-zinc-400 hover:text-zinc-200'
          }`}
        >
          <Layers className="w-4 h-4" />
          <span>Cursos ({db.courses.length})</span>
        </button>
        ...
        {activeCourse && (
          <button
            ...
            className={`flex flex-col items-center gap-1 px-3 py-1 rounded-lg text-[10px] font-semibold transition-colors cursor-pointer ${
              currentView === 'player' ? 'text-emerald-400 font-bold' : 'text-zinc-400 hover:text-zinc-200'
            }`}
          >
        ...
        <button
          ...
          className={`flex flex-col items-center gap-1 px-3 py-1 rounded-lg text-[10px] font-semibold transition-colors cursor-pointer ${
            currentView === 'stats' ? 'text-emerald-400 font-bold' : 'text-zinc-400 hover:text-zinc-200'
          }`}
        >
        ...
        <button
          type="button"
          onClick={() => setIsAddModalOpen(true)}
          className="flex flex-col items-center gap-1 px-3 py-1 rounded-lg text-[10px] font-semibold text-emerald-400 hover:text-emerald-300 transition-colors cursor-pointer"
        >
